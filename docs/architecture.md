...
Vendored CLIs, build/test executables, the external Codex event callback, in-process mounts, and the private WebWorker preview do not launch Harness applications. [`verify-application-entrypoints`](../scripts/verify-application-entrypoints.ts) classifies bins, executable sources, demos, and Web launch scripts; it rejects application launches outside `dsh`. The separate Team Battle macOS distribution also selects a `dsh` profile.
