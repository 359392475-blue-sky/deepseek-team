...
describe('Chat apply wiring', () => {
  ...
  it('contributes Chat View, node renderers, and stats', async () => {
    ...
    expect(b.runtime.slots.spec('conversation.chat.sidecar'))
  ...
  it('activates a deferred sidecar registration under the Chat declaration', async () => {
    const b = await bench()
    const Sidecar = (): null => null
    const contribution = b.runtime.ctx.plugin({
      name: 'chat-sidecar-probe',
      inject: ['slots'],
      apply: (ctx) => {
        ctx.slots.inject('conversation.chat.sidecar', () =>
          ctx.slots.register({ name: 'conversation.chat.sidecar' }, Sidecar))
      },
    })
    await contribution.await()

    expect(b.runtime.slots.entries('conversation.chat.sidecar').map(entry => entry.component))
      .toEqual([Sidecar])

    await b.chat.dispose()
    await Promise.resolve()
    expect(b.runtime.slots.spec('conversation.chat.sidecar')).toBeUndefined()
    expect(b.runtime.slots.entries('conversation.chat.sidecar')).toHaveLength(0)
