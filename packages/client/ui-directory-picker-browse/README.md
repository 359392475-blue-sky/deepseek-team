...
The package also provides the `workspace.directoryFlow` factory declared by [ui-workspace](../ui-workspace/README.md#the-directory-flow-hole), so independent client entries reuse this browse dialog. The factory registration leaves with its plugin fiber.
