...
| `sharedServer` | unset | Fixed Host creation destination `{ url, accessTokenRef }`; credentials remain on the Host and requests cannot override it. |
...
No runtime invariant companion is published: project progress and file review views derive from the stored aggregates, and durable reads and writes validate ownership, references, and capacity. The service keeps no independently updated copy of those projections to compare at runtime.

<a id="shared-server-teams"></a>
## Shared-server teams

`teams()` lists locally known spaces and hosting status without making network requests; `summary({ teamId })` fetches current member access states for handoff and roster filtering. Only the owner sees invitation details and invited-member expiry times. Historical identities remain in `view().members` for attribution; current rosters and online counts must filter them with the summary’s active member ids. `createTeam({ name, goal, memberName, memberRole })` uses the configured `sharedServer` and resolves its creation credential on the Host for each operation. It creates server-owned state and keeps only connection metadata and a member credential on this device; neither the creation credential nor its value enters browser responses or team storage. A configured Host rejects client-supplied server overrides. Without `sharedServer`, advanced callers may supply `serverUrl` and `serverAccessToken`; omitting both creates a local owner-only team. New teams contain no simulated colleagues.

The owner creates a member-bound invitation with `createInvite`, naming the invited person and role. The returned HTTPS link (private HTTP for local deployments) stores the team id and secret in its fragment, so opening it does not send the secret in a request URL. The join form also accepts previously issued `dsh-team://join` codes. A configured Host accepts invitations only for its fixed server; credential-bearing queries, redirects, and ambiguous server paths are rejected. `joinRemote({ inviteCode })` generates a device credential, redeems the invitation, and stores the credential through `ctx.credentials`. The server stores only credential and invitation digests. Invitations expire after 24 hours by default (request range: 1–168 hours), are single-use, and can be revoked; retrying the same redemption with the same credential is idempotent. `revokeMember` blocks subsequent member operations and preserves historical attribution. Revoking an invitation alone does not revoke an already joined member. An expired or removed member can use the existing join entry with a fresh invitation on the same device: the previous credential must first receive an explicit access rejection from the same server. Pasting a same-team invitation on a device with active membership opens its existing identity after a live server check, without redeeming the invitation or changing roles; this also applies to the owner and retained expired or revoked invitations. An owner identity or a failed network request prevents replacement. The invitation assigns a new member identity; old task and file attribution remains unchanged, and the owner can hand unfinished tasks to the new member.

`bindWorkspace({ teamId, workspaceId })` explicitly associates an accessible team with a registered local workspace, replacing that workspace’s prior association. `workspaceLinks()` lists associations whose workspace is still registered. These links persist in the separate local `team_battle_workspace_links` domain; they publish neither workspace paths nor private Session content and cannot be called through the shared `/call` route.
