...
const standardReleaseMemberDirectory
  = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/(?!(?:desktop(?:-host)?|macos)$)[^/]+|vendor\/[^/]+)$/
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (dir !== 'apps/macos' && dir.startsWith('apps/') && dir !== desktopApplicationDirectory && manifest.name?.startsWith('@deepseek-ai/')) {
